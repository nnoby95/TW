...
var VILL1ST="";
var ALTBOT=true;;
var VERZIO="4.51113"; /* (4.)+(kieg.-ek száma)+(alverz.(dátum))*/
$.getScript('http://cncdani2.freeiz.com/SZEM4/SZEM4_VER.js');
AZON=game_data.player.id+"_"+game_data.world+AZON;
}catch(e){alert(e);}

function vercheck(){try{
	if (typeof(CVERZIO)=="undefined") {naplo("Globál","Verzióellenõrzés sikertelen: nem érhetõ el a cnc weboldal"); return;}
...
function pic(file){
	return "http://cncdani2.freeiz.com/SZEM4/"+file;
}
...
function playSound(hang){try{
	...
	var play="http://cncdani2.freeiz.com/SZEM4/"+hang+".wav";
...
var BOTORA, ALTBOT2=false, BOT_VOL=0.0;
function BotvedelemBe(){try{
	...
	if (!ALTBOT2) {window.open(document.getElementById("altbotURL").value);ALTBOT2=true;}
...
function szem4_farmolo_1kereso(){try{/*Farm keresi párját :)*/
	...
	for (var i=1;i<a.length;i++){
		...
		for (var j=1;j<b.length;j++){
			...
			sp=sp*(Math.abs(Math.sqrt(Math.pow(parseInt(a[i].cells[0].textContent.split("|")[0])-parseInt(b[j].cells[0].textContent.split("|")[0]),2)+Math.pow(parseInt(a[i].cells[0].textContent.split("|")[1])-parseInt(b[j].cells[0].textContent.split("|")[1]),2))));/*a[i]<->b[j] távkeresés*/
...
function szem4_farmolo_2illeszto(adatok){try{ /*+: Min sereg, fal alapján*/
...
function szem4_farmolo_3egyeztet(adatok){try{
	...
	try{
		...
		if (adatok[6]!=a) debug("farm3","<b>NEM JÓ</b>");
	...
	if (adatok[1].indexOf("semmi")==-1 || adatok[6]>hatszam*2) {
		...
		FARM_REF.document.forms[0].submit.click();
		...
		if (adatok[0]>parseInt(opts[3].value)){
			...
			var ido=Math.abs(Math.sqrt(Math.pow(parseInt(adatok[2].split("|")[0])-parseInt(adatok[3].split("|")[0]),2)+Math.pow(parseInt(adatok[2].split("|")[1])-parseInt(adatok[3].split("|")[1]),2)));
...
function szem4_farmolo_4visszaell(adatok){try{
	...
	*/
	...
	/*Van e seregmozgás a faluban?*/
	try{
		var mozgas=false;
		var seregmozgas=FARM_REF.document.getElementById("content_value").getElementsByTagName("table");
		var patt = new RegExp(/[0-9]+(\|)[0-9]+/);
		try{
			if (patt.test(seregmozgas[seregmozgas.length-2].rows[1].cells[0].textContent) && seregmozgas[seregmozgas.length-2].rows[0].cells.length>2) {mozgas=true; seregmozgas=seregmozgas[seregmozgas.length-2].rows;}
		}catch(e){}
		try{
			if (!mozgas)
			if (patt.test(seregmozgas[seregmozgas.length-1].rows[1].cells[0].textContent) && seregmozgas[seregmozgas.length-1].rows[0].cells.length>2) {mozgas=true; seregmozgas=seregmozgas[seregmozgas.length-1].rows;}
		}catch(e){}
	}catch(e){debug("4_seregmozgas",e); mozgas=false;}
	...
	for (var i=0;i<a.length;i++){
		if (i==4) continue;
		if (a[i].checked && E_SEB[i]>fastest) fastest=E_SEB;
	}
	
	var sarga=true; var plusz=60;
	if (mozgas){
		var koord; var tav; var h_ido; var natt=false;
		for (var i=1;i<seregmozgas.length;i++){
			if (seregmozgas[i].cells[0].getElementsByTagName("img")[0].src.indexOf("support")>-1) continue;
			if (seregmozgas[i].cells[0].getElementsByTagName("img")[0].src.indexOf("attack")==-1) {natt=true;}
			koord=seregmozgas[i].cells[0].textContent.match(/[0-9]+(\|)[0-9]+/g);
			koord=koord[koord.length-1];
			if (!natt){
			tav=Math.abs(Math.sqrt(Math.pow(parseInt(koord.split("|")[0])-parseInt(adatok[2].split("|")[0]),2)+Math.pow(parseInt(koord.split("|")[1])-parseInt(adatok[2].split("|")[1]),2)));
			tav=tav*fastest*(1/UNIT_S)*(1/SPEED);
			} else tav=0;
			h_ido=seregmozgas[i].cells[2].textContent.split(":"); h_ido=parseInt(h_ido[0]*60)+parseInt(h_ido[1]);
			tav+=h_ido;
			if (tav<plusz) plusz=tav;
			if (natt) break;
		}
	}
...
function szem4_VIJE_1kivalaszt(){try{
	...
	for (var i=VT.length-2;i>0;i--){
		...
		var oraperc=VT[i].cells[2].textContent.match(/[0-9]+:[0-9]+/g)[0];
		var nap=VT[i].cells[2].textContent.replace(oraperc,"").match(/[^0-9][0-3][0-9][^0-9]/g)[0].match(/[0-3][0-9]/g)[0];
		d.setMinutes(oraperc.split(":")[1]);
		d.setHours(oraperc.split(":")[0]);
		d.setDate(nap);
		/*debug("VIJE1()","Ezt nézem: koord "+koord+". Idõ: "+d+", most: "+d2+". Különbség: "+(d2-d));*/
		if ((d2-d)>10800000) var regi=true; else var regi=false;
...
function szem4_VIJE_motor(){try{
	...
	if (VIJE_LEPES!=0 && VIJE_REF1.closed) VIJE_LEPES=0;
